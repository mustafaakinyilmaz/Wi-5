...
def mkdir_click():
    ...
    ssh.connect(remotehost, port=port, username=username, password=password)
    try:
        ssh.exec_command('mkdir /usr/share/click')
        print('click directory created under /usr/share')
    except:
        print('not created')
...
def ssh_connection_restart_network():
    ...
    try:
        ...
        try:
            ...
            channel2.exec_command('chmod +x /tmp/agent_click_forAR71xx')
            channel3.exec_command('/etc/init.d/network restart')
            print("init.d and chmod +x successful")
            print("Current ip is: %s" % (E2.get()))
            messagebox.showinfo("IP","Please give your PC a static IP with domain %s." %(E2.get()[8]))
        except:
...
# Go Button
GoButton = tkinter.Button(master,text = "GO!", command = lambda: [configure_wireless(),configure_start_sh(),configure_network(),configure_agent_click(),config_message(),mkdir_click(),transfer_configurations(),ssh_connection_restart_network()])
